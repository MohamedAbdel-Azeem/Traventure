...
import {
  rateItinerary,
  rateTourGuide,
  isUserBookedInItineraryOFtourGuide,
  rateActivity,
  IFeedbackk,
  getAllTourGuideReviews,
  getAllActivityReviews,
  getAllItineraryReviews,
} from "../Model/Queries/feedback_queries";
...
router.get("/canfeedback", async (req, res) => {
  ...
  try {
    ...
    const canProvideFeedback = await isUserBookedInItineraryOFtourGuide(
      touristId,
      tourGuideUserId
    );
