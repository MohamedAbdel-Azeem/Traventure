  ...
  const bookHotel= async (hotel:any,tourist_username:string) => {
    ...
    try {
        ...
        const response = await axios.post(url, {
          ...
          ...(hotel.offers?.[0]?.room?.typeEstimated?.bedType && { bedType: hotel.offers?.[0]?.room?.typeEstimated?.bedType }),
            ...(hotel.offers?.[0]?.room?.typeEstimated?.category && { roomType: hotel.offers?.[0]?.room?.typeEstimated?.category }),
