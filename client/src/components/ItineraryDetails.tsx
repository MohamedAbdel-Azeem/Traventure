...
const ItineraryDetails: React.FC = () => {
  ...
  const handleTagsText = (value: string[]) => {
    const valueNames = editedItinerary.selectedTags
  ...
  const handleAddTag = (event: React.MouseEvent<HTMLButtonElement>): void => {
    ...
    if (newTag && !selectedTags.includes(newTag)) {
      ...
      setEditedItinerary({
        ...editedItinerary,
        selectedTags: tagsOptions.filter(tag => updatedTags.includes(tag._id)) // Update with tag objects
      });
    }
  ...
  const handleRemoveTag = (tagToRemove: TagStructure) => {
    setEditedItinerary((prev) => ({
      ...prev,
      selectedTags: prev.selectedTags?.filter((tag) => tag._id !== tagToRemove._id),
    }));
  };
  ...
  const handleAddPlace = () => {
    ...
    setEditedItinerary({ ...editedItinerary, plan: [...editedItinerary.plan, { place: newPlace, activities: [] }] });
  };

  const handleRemovePlace = (placeIndex: number) => {
    const updatedPlan = editedItinerary.plan.filter((_, index) => index !== placeIndex);
    setEditedItinerary({ ...editedItinerary, plan: updatedPlan });
  };
  const handleActivityChangetime_unit = (
    ...
    //   .value as string;
    updatedPlaces[placeIndex].activities[activityIndex].time_unit = e.target.value;
    setEditedItinerary({ ...editedItinerary, plan: updatedPlaces });
    console.log("editing",editedItinerary);
  };
const handleActivityChangeDuration=(planindex: number, activityIndex: number, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>)=> {
  const updatedPlaces = [...editedItinerary.plan];
  updatedPlaces[planindex].activities[activityIndex].activity_duration = e.target.value;
  setEditedItinerary({ ...editedItinerary, plan: updatedPlaces });
  console.log("editing",editedItinerary);
}
  ...
  const handleActivityChangeID = (
    ...
    const updatedPlaces = [...editedItinerary.plan];
    updatedPlaces[placeIndex].activities[activityIndex].activity_id = e.target.value;
    setEditedItinerary({ ...editedItinerary, plan: updatedPlaces });
    
  };

  const handlePlaceChangeID = (planIndex: number, e: SelectChangeEvent<string>) => {
    const selectedPlaceId = e.target.value;
    const selectedPlace = apiPlaces?.find((place) => place._id === selectedPlaceId);
          ...
          <Dialog open={isEditing} onClose={handleCloseModal}>
            ...
            <DialogContent>
              <TextField
                ...
                onChange={(e) =>{
                  setEditedItinerary({ ...editedItinerary, title: e.target.value })
                  console.log("editing",editedItinerary);}
                }
              />
              <TextField
  ...
  <InputLabel id={`place-label-${planIndex}`}>Select Place</InputLabel>
  <Select
  ...
  <InputLabel id={`activity-label-${planIndex}-${activityIndex}`}>
    Select Activity
  </InputLabel>
  <Select
    labelId={`activity-label-${planIndex}-${activityIndex}`}
    value={activity.activity_id } // Ensure value is a string
    onChange={(e) => handleActivityChangeID(planIndex, activityIndex, e)}
  >
...
<Box>
  ...
  <Select
    ...
    multiple
    value={editedItinerary.selectedTags?.map((tag) => tag._id) || selectedTags}
